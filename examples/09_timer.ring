...
func onResetTimer
    ...
    ok
    nSeconds = 0
    oApp.setNumber("seconds", 0)
    oApp.setBool("running", false)
    oApp.setString("status", "Reset")
    ? "Timer reset"

func onTick
    nSeconds++
    oApp.setNumber("seconds", nSeconds)
    ? "Tick: " + nSeconds
...
func onCheckRunning
    if nTimerId != 0
        ...
        if bRunning
            oApp.setString("status", "Timer IS running")
            ? "Timer " + nTimerId + " is running"
        else
            oApp.setString("status", "Timer exists but NOT running")
            ? "Timer " + nTimerId + " exists but is not running"
        ok
    else
        oApp.setString("status", "No timer created")
...
func onRestartTimer
    if nTimerId != 0
        ...
        oApp.setBool("running", true)
        oApp.setString("status", "Restarted")
    else
        nSeconds = 0
        oApp.setNumber("seconds", 0)
        nInterval = oApp.getProperty("interval-ms")
...
func onChangeInterval
    ...
    if nTimerId != 0
        ...
        oApp.setString("status", "Interval changed to " + nInterval + "ms")
        ? "Timer " + nTimerId + " interval changed to " + nInterval + "ms"
    else
        ? "No timer to modify - will use new interval when started"
        oApp.setString("status", "Will use " + nInterval + "ms when started")
    ok

func onOnceTimer
    oApp.setString("status", "One-shot timer fired!")
