...
func onCelsiusChanged
    ...
    oApp.setNumber("fahrenheit", round(nFahrenheit, 2))
    oApp.setNumber("kelvin", round(nKelvin, 2))
    updateBar(nCelsius)
...
func onFahrenheitChanged
    ...
    nKelvin = nCelsius + 273.15
    oApp.setNumber("celsius", round(nCelsius, 2))
    oApp.setNumber("kelvin", round(nKelvin, 2))
    updateBar(nCelsius)
...
func updateBar nCelsius
    ...
    oApp.setNumber("temp-percent", nPercent)
    if nCelsius < 0
        oApp.setString("temp-label", "Freezing! 🥶")
    but nCelsius < 15
        oApp.setString("temp-label", "Cold 🧊")
    but nCelsius < 25
        oApp.setString("temp-label", "Comfortable 😊")
    but nCelsius < 35
        oApp.setString("temp-label", "Warm ☀️")
    else
        oApp.setString("temp-label", "Hot! 🔥")
    ok
